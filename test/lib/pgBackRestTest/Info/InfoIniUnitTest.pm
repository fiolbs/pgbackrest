...
sub run
{
    ...
    if ($self->begin("iniParse()"))
    {
        ...
        $self->testResult(sub {iniParse($strIni, true)}, '{section => {key => value}}', 'relaxed value read');

        #---------------------------------------------------------------------------------------------------------------------------
        $strIni = "[section]\nkey=value1\nkey=value2\nkey=value3";

        $self->testResult(sub {iniParse($strIni, true)}, '{section => {key => (value1, value2, value3)}}', 'relaxed array read');
    }
